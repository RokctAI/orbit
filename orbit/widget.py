...
class LoginDialog(tk.Toplevel):
    def __init__(self, parent, theme, on_success):
        ...
        tk.Label(
            self,
            text="Connect to Gravity Server",
    ...
    def submit(self):
        ...
        if not server or not token:
            messagebox.showerror(
                "Error", "Both Server URL and API Token are required.", parent=self
            )
            return
...
class TokenStatusWidget:
            ...
            if not self.country_code:
                try:
                    ...
                    req = urllib.request.Request(
                        "http://ip-api.com/json", headers={"User-Agent": "Orbit/1.0"}
                    )
            ...
            if self.country_code:
                ...
                if not os.path.isfile(self.flag_path):
                    try:
                        ...
                        req = urllib.request.Request(
                            flag_url, headers={"User-Agent": "Orbit/1.0"}
                        )
    ...
    def update_layout(self, is_done=True):
        ...
        if self.is_online:
            if (
                bg.lower() in ["#a6e3a1", "#13f300"]
                or bg.lower().startswith("#a")
                or bg.lower().startswith("#0")
                or bg.lower().startswith("#13")
            ):
                color = "#ffffff"
            else:
                color = "#13F300"
            ...
            if (
                bg.lower() in ["#ff4600", "#f38ba8"]
                or bg.lower().startswith("#f")
                or bg.lower().startswith("#e")
            ):
                color = "#ffffff"
            else:
        ...
        if self.is_compact:
            ...
            self.token_label.configure(
                text=f"{self.tokens_str} ({self.size_str.replace(' ', '')})"
            )
            self.token_label.pack(side=tk.LEFT)
        else:
            # Expanded Layout
            ...
            self.sep_label1.pack(side=tk.LEFT, padx=6)
            self.token_label.configure(
                text=f"Tokens: {self.tokens_str} ({self.size_str})"
            )
            self.token_label.pack(side=tk.LEFT)
        ...
        target_height = (
            self.height + self.todo_height if self.todo_expanded else self.height
        )
    ...
    def refresh_todo_list(self):
        ...
        for idx, t in enumerate(todos):
            ...
            row.bind(
                "<Double-Button-1>",
                lambda e, i=idx: [self.toggle_todo_item(i), "break"][1],
            )
